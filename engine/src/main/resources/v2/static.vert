...
layout (std140) uniform Light {
      vec3 light_color;   //0
      vec3 light_position; //16
      vec3 light_reflecitvity; //32
};
...
void main(void){
    ...
    //vector to light
    vec3 toLightVector = normalize(light_position.xyz - worldCoords.xyz);

    //vector from vertex to viewer
    vec3 toCameraVector = normalize((inverse(view) * vec4(0.0,0.0,0.0,1.0)).xyz - worldCoords.xyz);

    //final intensity
    lightIntensity = light_color.xyz * light_reflecitvity.xyz * max( dot( toLightVector, surfaceNormal), 0.2);
    pass_uv = uv;
}
